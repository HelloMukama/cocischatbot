...
# for a sort of refined way of stemming words that takes the root of the word and not a truncated
# formd
...
model.add(Dropout(0.5))
# add the last output layer with softmax function that allows us to sum up the percentages to add
# up to 1 and show the likelyhood of each neuron being the right choice
model.add(Dense(len(train_y[0]), activation='softmax'))

# adding stochastic gradient descent optimizer to find the model parameters that correspond to
